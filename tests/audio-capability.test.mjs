...
function primeModule() {
  const fn = [
    ...
    varDecl("audioCapPriming"), src("primeAudioCapability")
  ].join("\n");
  return new Function("navigator", "window", "screen", "localStorage", "audioCapability",
...
test("the probe stack always builds the FULL route — asking richAudio() inside it would be circular", () => {
  ...
  /* One Reverb/FeedbackDelay/Chorus each in SOURCE, inside a room()/chain()/voice() helper that
     ...
     unrolled four times. */
  assert.match(fn, /function room\(decay\) \{ return new Tone\.Reverb/);
